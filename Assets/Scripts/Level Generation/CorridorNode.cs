...
internal class CorridorNode : Node
{
    ...
    {
        var realativePositionOfStructure2 = CheckPositionStucture2AgainstStructure1();
        switch (realativePositionOfStructure2)
        {
    ...
    private void ProcessRoomInRelationRightOrLeft(Node structure1, Node structure2)
    {
        ...
        int y = GetValidYForNeighborLeftRight(leftStructure.TopLeftAreaCorner, leftStructure.BottomRightAreaCorner, rightStructure.TopLeftAreaCorner, rightStructure.BottemLeftAreaCorner);
        while( y == -1 && sortedLeftStructure.Count > 1)
        {
            ...
            leftStructure = sortedLeftStructure[0];
            y = GetValidYForNeighborLeftRight(leftStructure.TopLeftAreaCorner, leftStructure.BottomRightAreaCorner, rightStructure.TopLeftAreaCorner, rightStructure.BottemLeftAreaCorner);
        }
        BottemLeftAreaCorner = new Vector2Int(leftStructure.BottomRightAreaCorner.x, y);
    ...
    private int GetValidYForNeighborLeftRight(Vector2Int leftNodeUp, Vector2Int leftNodeDown, Vector2Int rightNodeUp, Vector2Int rightNodeDown)
    {
        ...
        if(rightNodeUp.y <= leftNodeUp.y && leftNodeDown.y <= rightNodeUp.y)
        {
    ...
    private void ProcessRoomInRelationUpOrDown(Node structure1, Node structure2)
    {
        ...
        var possibleNeighborsInTopStructue = structureaboveChildren.Where(child => GetValidXForNeighborUpDown(
            bottomStructure.TopLeftAreaCorner, bottomStructure.TopRightAreaCorner, child.BottemLeftAreaCorner, child.BottomRightAreaCorner) != -1).OrderBy(ChildrenNodeList => ChildrenNodeList.BottomRightAreaCorner.y).ToList();
        if(possibleNeighborsInTopStructue.Count == 0)
        {
    ...
    {
        Vector2 middlePointStructure1Temp = ((Vector2)structure1.TopRightAreaCorner + structure1.BottemLeftAreaCorner) / 2;
        Vector2 middlePointSturcture2Temp = ((Vector2)structure2.TopRightAreaCorner + structure2.BottemLeftAreaCorner) / 2;
        float angle = CalculateAngle(middlePointStructure1Temp, middlePointSturcture2Temp);
    ...
    private float CalculateAngle(Vector2 middlePointStructure1Temp, Vector2 middlePointSturcture2Temp)
    {
        return Mathf.Atan2(middlePointSturcture2Temp.y -  middlePointStructure1Temp.y, middlePointSturcture2Temp.x -  middlePointStructure1Temp.x) * Mathf.Rad2Deg;
    }
}